...
class Predictor(object):
    ...
    def predict_embeddings(self, test_prot, cmap_thresh=10.0, chain='query_prot', layer_name = 'labels'):
        """
        Method for extracting embeddings from specific layer using keract library.
        """
        
        # print ("### Computing embeddings on a single protein...")
        # self.model.summary()
        # batch = []

        # for i in range(0, batch_size):
        #     if self.gcn:
        #         A, S, seqres = self._load_cmap(test_prot[i], cmap_thresh = cmap_thresh)
        #         batch.append([A, S])
        #     else:
        #         S = seq2onehot(str(test_prot[i]))
        #         S = S.reshape(1, *S.shape)
        #         batch.append(S)

        # batch = list(batch)
        # embeddings = get_activations(self.model, batch, layer_names = layer_name)
        # embeddings = embeddings[layer_name]
        # embeddings = embeddings.reshape(-1)

        if self.gcn:
            A, S, seqres = self._load_cmap(test_prot, cmap_thresh=cmap_thresh)
            embeddings = get_activations(self.model, [A, S], layer_names = layer_name)
        else:
            S = seq2onehot(str(test_prot))
            S = S.reshape(1, *S.shape)
            embeddings = get_activations(self.model, S, layer_names = layer_name)
